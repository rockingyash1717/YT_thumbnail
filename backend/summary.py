...
def download_video(video_url, download_dir="downloads"):
    """Download a video from YouTube to a specific directory and return its file path."""
    # Ensure the download directory exists
    os.makedirs(download_dir, exist_ok=True)

    ydl_opts = {
        'format': 'bestvideo+bestaudio[ext=mp4]',  # Download best video and audio as mp4
        'outtmpl': os.path.join(download_dir, '%(title)s.%(ext)s'),  # Save to the specified directory
        'merge_output_format': 'mp4',  # Ensure final file is saved as mp4
    }

    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        info = ydl.extract_info(video_url, download=True)  # Extract video information and download
        video_filename = ydl.prepare_filename(info)  # Get the generated filename
        return video_filename  # Return the path of the downloaded video
...
if __name__ == '__main__':
   ...
   text=process_video_from_url("https://youtu.be/yWQLaqyTVds?si=FMMvDDnJ60eVMB8y")
   print(text)